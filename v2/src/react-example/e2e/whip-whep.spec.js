...
/*
 * The HTTP signalling path. WHIP for ingest, WHEP for egress.
 *
 * This is a separate code path from the WebSocket one inside startPublish.js and
 * startPlay.js, with its own peer connection setup and its own failure modes, so it needs
 * its own coverage rather than being assumed to work because wss does.
 */
...
test.describe('WHIP ingest', () => {
  ...
  test('records the WHIP HTTP exchange in the debug panel, not the socket', async ({ page }) => {
    ...
    /*
     * The WHIP/WHEP channel specifically, not just "some rows". This assertion used to be
     * `.wz-debug__row` count > 0, which passed on the peer-connection rows alone while the
     * HTTP exchange was not being recorded at all - logHttp existed, was unit tested, and
     * was never called from the WHIP flow.
     */
