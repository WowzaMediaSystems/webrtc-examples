...
/*
 * Data channels: chat in both directions, and captions from publisher to player.
 *
 * Both features are gated on the Engine supporting them (4.12.0 or later per the form's
 * own note), so a failure here can mean either the example or the Engine. The assertions
 * say which side was observed.
 */
...
test.describe('data channel: captions', () => {
  // Captions are not typed. The publisher opens a second data channel and cycles a fixed
  // pool of placeholder lines on a 3 second timer; players render them as subtitles. So
  // the test waits for a known phrase rather than sending one.
