 ...
 *
 * Collapsed it is icons only; expanded it shows the label beside each icon. The utility
 * links (docs, portal, source) sit at the foot, below a rule, so they never compete with the
 * page links for the same reading position.
 *
 * Every item is a real link or button, so the rail is reachable by keyboard in both states,
 * and the icon-only state carries an aria-label.
 *
 * Connection status is not here: it belongs in the stage's topbar, beside the stream it
 * describes, rather than at the far left of the window.
 */

/* A camera, because this page is where the video comes from. An upload arrow described the
   direction of travel but said nothing about the source being a capture device. */
const PublishIcon = () => (
