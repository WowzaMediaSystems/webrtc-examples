...
/*
 * Per-rendition readout for a simulcast publish.
 *
 * Simulcast sends several encodings at once and the browser reports one outbound-rtp for
 * each. A single figure cannot describe that, and picking one at random is worse than
 * saying nothing: it reads as the whole publish.
 *
 * A layer at zero is a normal and important state, not an error. Chromium drops encodings
 * the bandwidth estimate will not carry, and refuses ones whose scaled size falls below its
 * own floor, so the reason is shown beside the layer rather than left to be guessed at.
 *
 * Rungs read top down, highest first. That order has to come from the configuration: the
 * browser does not report scaleResolutionDownBy on outbound-rtp, so a rung that is not
 * sending has no frame size to be ranked by and sorting on the stats alone gave alphabetical
 * order - h, l, m - which is not the ladder anyone configured.
 */
