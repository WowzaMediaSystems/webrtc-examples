...
import { clearLog, subscribe } from '../../diagnostics/signalLog';
import Resizer from '../shell/Resizer';
import { useResizable } from '../../hooks/useResizable';

/*
 * Collapsible view of the signaling conversation with the Engine. Collapsed by default:
 * it is a diagnostic aid, not part of the main task, and it should not be the first thing
 * a customer sees on the page.
 *
 * Open, it is resizable from its top edge, because how much log you want to see depends
 * entirely on what you are chasing.
 */

/*
 * `empty` explains a channel with nothing in it. A blank panel reads as broken, when most
 * of the time it means the session simply did not use that transport: a WHIP publish opens
 * no socket, and a wss publish makes no HTTP requests.
 */
const CHANNELS = [
...
const DebugPanel = ({ defaultOpen = false }) => {
  ...
  // Never more than two thirds of the window: the stage above it has to stay usable.
  ...
  /*
   * Newest first. The interesting line during a live session is the one that just arrived,
   * and hunting for it at the bottom of a growing list is the wrong way round.
   */
