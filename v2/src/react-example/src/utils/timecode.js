/*
 * A timestamp burned into the picture, so latency can be measured end to end.
 *
 * Why in the picture and not over a data channel: a message sent alongside a frame does not
 * travel with it. It skips capture, encode, the encoder's own queue, decode and display, so
 * it arrives early and reports a latency nobody can see on screen. A timestamp drawn into
 * the frame goes through every one of those stages, which is what "glass to glass" means.
 *
 * The encoding is deliberately crude, because it has to survive being scaled down to a
 * 480-wide rendition and squeezed through a lossy codec: flat black and white blocks, sized
 * as a fraction of the frame so the reader can find them at any resolution, read at their
 * centres where compression does least damage, and judged against two reference blocks
 * carried in the same row rather than against an absolute threshold.
 *
 * The value is milliseconds modulo 65536, which wraps every 65.5 seconds. The reader
 * resolves the wrap against its own clock, so anything up to about 32 seconds of latency
 * reads correctly, and anything beyond that is not a number worth reporting anyway.
 */
...
/**
 * Reads the pattern back.
 *
 * `sample(index)` returns the luminance at the centre of block `index`, 0-255, or null.
 * Returns null when the reference blocks do not look like a reference pair, or when the
 * check nibble disagrees: a wrong number here would be worse than no number.
 */
export const decodeTimecode = (sample) => {
  ...
  // The references have to be clearly apart, or this is not a timecode at all, just
  // whatever happened to be in the corner of the picture.
...
/**
 * Turns a wrapped reading into a latency in milliseconds.
 *
 * The reader knows roughly what time it is, so the only ambiguity is which 65.5 second
 * window the reading came from, and the nearest one is the answer. Readings that imply a
 * negative delay, or an implausible one, are rejected rather than clamped: a clock that
 * disagrees should produce no measurement, not a flattering one.
 */
...
/**
 * Draws the pattern, and a readable clock beneath it, onto a 2D context.
 *
 * The readable clock is for a person looking at two screens side by side; the blocks are
 * for the reader. They carry the same instant.
 */
...
/** Where the reader should sample: the centre of block `index`. */
