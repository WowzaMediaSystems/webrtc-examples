...
/*
 * A stand-in for RTCPeerConnection that deliberately does NOT dispatch
 * connectionstatechange on close(), because Chromium does not either. Verified in the
 * browser: a connection closed by hand went from "new" to "closed" with the listener never
 * called. That is the whole reason releasePeerConnection exists.
 */
...
/*
 * An abandoned attempt releases too. Without naming the connection it meant, an attempt that
 * times out after a newer session has already started deregisters the live one, and the stats
 * panel blanks on a session that is working perfectly.
 */
describe('releasing the connection you meant', () => {
  ...
  // The stop buttons mean "whatever is current", and say so by naming nothing.
