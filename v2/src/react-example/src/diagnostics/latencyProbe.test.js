...
/*
 * The state shape summarizeProbe reads. Spelled out here rather than imported, because the
 * module's own createState is private and a test that can only build state through the
 * production path cannot reach the statuses that matter.
 */
...
/*
 * A round trip against a far end whose clock is offsetMs ahead of ours, replying instantly so
 * its turnaround cancels out and the round trip is exactly rttMs. Same construction as
 * clockSync.test.js, because the two modules have to agree on what a sample is.
 */
...
describe('decideStamping', () => {
  ...
  /*
   * The trap this function exists for. An SEI NAL prepended to a VP8 frame is not ignored at
   * the far end, it corrupts the frame, so a non-H.264 codec must be a refusal and not a
   * "measure anyway".
   */
...
describe('stampFrameBytes', () => {
  /*
   * The sender and the receiver halves meeting in the middle, over the real frameStamp codec:
   * what the transform writes is what findSeiPayload reads back, and the original frame is still
   * there behind it.
   */
...
describe('describeClock', () => {
  ...
  /*
   * One clock is provable when the publisher is stamping in this same page, and only then. A
   * near-zero offset on its own does not say where the far end runs.
   */
  ...
  /*
   * The refusal the design asks for by name: a confidently wrong latency is the failure this
   * whole feature exists to delete, so above the threshold there is no number at all.
   */
...
describe('summarizeProbe', () => {
  ...
  /*
   * The case an operator will hit most often, and the one that must never look like a bug:
   * frames are flowing and none of them carry a stamp.
   */
...
describe('the clock messages', () => {
  ...
  /*
   * The publisher answers on a channel the Engine mirrors to every viewer, so one viewer's
   * reply reaches all of them. Sequences start at zero on each player, so without the id two
   * viewers of one stream take each other's replies as their own and estimate the offset from
  ...
  /*
   * The split-view page runs the responder and the initiator in one browser on one channel, so
   * a responder that answered replies would answer itself and the round trip would be garbage.
   */
...
describe('availability', () => {
  /*
   * jsdom has no RTCRtpSender, which is the same shape of answer a non-Chromium browser gives:
   * unavailable, with a reason, and nothing half-attached.
   */
...
describe('the subscription', () => {
  /*
   * Null, not an object full of nulls. A subscriber has to be able to tell "no stamped frame has
   * ever arrived" from "here is a latency", and an empty sample looks like the second.
   */
 ...
 * Date.now. The clock samples still travel out to the Engine and back, and that round trip put
 * the uncertainty past the trusted bound: measured at 34 ms, which made the page that most
 * testing runs on refuse to show any figure at all.
 */
describe('one page that both stamps and reads', () => {
  ...
  /*
   * The guard that keeps this honest. A combined page can be pointed at another machine's
   * stream, and then the frames carry that machine's clock however local the publishing is.
   */
