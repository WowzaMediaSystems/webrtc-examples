...
describe('HTTP logging', () => {
  it('marks 4xx and 5xx as errors', () => {
    ...
    expect(getEntries().at(-1).direction).toBe('info');
    logHttp('POST', '/whip', 503);
    expect(getEntries().at(-1).direction).toBe('error');
  });
});
describe('v2 protocol labelling', () => {
  // The live Engine keys every frame on messageType. Labelling these as a generic
  // 'message' made the debug panel far less useful, which only showed up against a
  // real server.
...
/*
 * Closing a socket on purpose still raises an error event, because the Engine can have a frame
 * in flight when we close and the browser then fails the connection with "Data frame received
 * after close". Reported as a failure, it put a red banner on the page every time a publish was
 * stopped.
 */
...
/*
 * A WebSocket error event carries no detail by design, so reading .message off it produced the
 * "Websocket Error: undefined" the panel used to show.
 */
