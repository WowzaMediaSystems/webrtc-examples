...
describe('diffSamples', () => {
  ...
  // The whole point: the entry has to carry both numbers, or it is no better than the tile.
  ...
  /*
   * The case the band test missed. 40 ms to 146 ms is a near-quadrupling that never leaves
   * the "good" band, so a crossing test alone reports nothing while the figure runs away.
   */
  ...
  // Usually the reason for the two above, so it has to be in the same log.
...
describe('createStatsWatcher', () => {
  ...
  /*
   * Drift is measured from the last thing said, not from one second ago, or a slow climb
   * is invisible: every individual step is too small to report.
   */
