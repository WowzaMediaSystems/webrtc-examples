...
/*
 * Simulcast. The shape here is what a live Chromium actually reported on 2026-09-17 for a
 * three-rung publish: one encoding sending, the other two configured, active, and at zero
 * bytes with qualityLimitationReason "bandwidth".
 */
...
describe('simulcast layers', () => {
  ...
  /*
   * The headline bitrate has to agree with the layer table under it. Reading one
   * outbound-rtp made the total smaller than the sum of its parts.
   */
  ...
  /*
   * A sender has no inbound stream, so the receiver's denominator does not exist for it.
   * Without the sender branch the publisher's packet loss was permanently a dash.
   */
