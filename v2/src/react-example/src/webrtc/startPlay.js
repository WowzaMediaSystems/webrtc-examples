...
const peerConnectionOnError = (error, callbacks) => {
  ...
  // The same sentence in both places. The log was corrected and this was not, so the banner
  // went on saying "PeerConnection Error: undefined" for an Event, which carries no message.
...
const websocketOnError = (error, callbacks) => {
   ...
   * A WebSocket error event also has no .message, which is how the panel came to say
