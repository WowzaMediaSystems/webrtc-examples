...
function enhanceSDP(sdpStr, enhanceData)
{
	...
	//
	// Remembering FireFox wants the bitrate entries in BPS, not
	// kps.
	...
	for(var sdpIndex in sdpLines)
	{
		...
		if ( sdpLine.indexOf("c=IN") ==0 )
		{
			...
			{
				sdpStrRet += "\r\nb=TIAS:"+(enhanceData.audioBitrate*1000)+"\r\n";
				sdpStrRet += "b=AS:"+(enhanceData.audioBitrate*1000)+"\r\n";
				sdpStrRet += "b=CT:"+(enhanceData.audioBitrate*1000)+"\r\n";
				continue;
			}
			if ('video'.localeCompare(sdpSection) == 0)
			{
				sdpStrRet += "\r\nb=TIAS:"+(enhanceData.videoBitrate*1000)+"\r\n";
				sdpStrRet += "b=AS:"+(enhanceData.videoBitrate*1000)+"\r\n";
				sdpStrRet += "b=CT:"+(enhanceData.videoBitrate*1000)+"\r\n";
				continue;
			}
		}
		sdpStrRet += '\r\n';
